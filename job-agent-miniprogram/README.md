...
- 首页：档案完整度、最佳匹配
- 经历：基本信息、技能、工作经历
- **智能添加岗位**：粘贴 JD、**从微信复制 BOSS 链接**、猎聘/小红书链接导入
...
微信**不允许**从 BOSS 直聘小程序直接跳转到 JobAgent，推荐流程：

1. BOSS 小程序 → 岗位详情 → 「···」→ 转发给「文件传输助手」
2. 微信聊天里 **长按链接 → 复制**（或复制整段 JD）
3. JobAgent → **岗位 → 智能添加** →「微信导入」→ **检测剪贴板并导入**

已在 `app.json` 声明 `getClipboardData`；请在微信公众平台 **设置 → 用户隐私保护指引** 中勾选「剪切板」并说明用途（导入岗位信息）。
