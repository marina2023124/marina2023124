...
2. 在 **SQL Editor** 中依次执行：
   ...
   - 若小程序报 `permission denied`，执行 [`supabase/miniprogram-permissions-fix.sql`](supabase/miniprogram-permissions-fix.sql)（一次性修复全部）
...
离职时打开 **离职备份与清理**（`/offboard`）：扫描云端个人材料、浏览器残留，并在工作电脑运行 `bash scan-leave.sh` 列出要备份 / 清理 / 不要带走的文件。个人履历可带走；周报原文和公司密钥不要带走。
